...
プラグインの機能の発火・検証方法, およびテストを実行するための前提環境を１つのファイル（シナリオ(ファイル)という）で表します。
Scenamatica はシナリオ（ファイル）を読み込み, その内容に沿ってプラグインの機能を発火し, その結果を検証・報告します。

例えば, とあるプラグインの「プレイヤが村人をキルすると」（**発火条件**） 
-> 「ダイアモンドがドロップする機能」（**目標動作**）をテストする方法について考えます。  

このテストは, Scenamatica の以下の機能を用いて達成されます。

1. [コンテキスト](/docs/use/scenario/elements#context) - [エンティティ](/docs/use/scenario/elements#entity)  
  この機能は, テストに必要なエンティティ（e.g. 村人, ガーディアン, 牛 等）をテストの**開始前に生成**, および**終了後に破棄**します。  
  発火条件にある村人をスポーンするのに使います。
2. [コンテキスト](/docs/use/scenario/elements#context) - [アクタ](/docs/use/scenario/elements#actor)  
  この機能は, テストに必要な専用のプレイヤ（アクタという）を生成します。  
  外部との通信を必要とせず, サーバ内で完結する, 特別なプレイヤのモックです。  
  発火条件にある村人をキルさせるのに使います。
3. [アクションの実行](/docs/use/scenario/elements#scenario-type-action-execution)
...
発火条件となる[アクションを実行](/docs/use/scenario/elements#scenario-type-action-execution)し,
それによってもたらされたプラグインの[動作を検証](/docs/use/scenario/elements#scenario-type-action-execution-expect)
することを繰り返すことで, シナリオテストが成立します。  
...
[アクションのフロー：シナリオ](/docs/use/scenario/elements#scenario) など, シナリオファイルの内容すべてを一般に指す語。
