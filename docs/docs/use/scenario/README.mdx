...
シナリオは, プラグインのテスト方法を *step-by-step* で定義したものです。

プラグインのテストを実行するための前提環境や, テスト対象機能の発火, および機能の検証方法を１つのファイル（シナリオファイルという）に記述します。  
Scenamatica は, プラグインに同梱されているシナリオファイルを読み込み, その内容に沿ってプラグインの機能を発火させ, 状態の遷移およびそれによる結果を検証・報告します。

ここでいう「テストを実行するための前提環境」とは, シナリオに登場する模擬的な[エンティティ](/docs/use/scenario/elements#entity)
や模擬的なプレイヤ（=[アクタ](/docs/use/scenario/elements#actor)）, テストのためのワールド（=[ステージ](/docs/use/scenario/elements#stage)）の３つを指します。
これらは, 同ファイルに定義される[コンテキスト](/docs/use/scenario/elements#context)によって提供されます。  

つまり, 発火条件となる[アクションを実行](/docs/use/scenario/elements#scenario-type-action-execution)し, それによってもたらされたプラグインの[動作を検証](/docs/use/scenario/elements#scenario-type-action-execution-expect)を繰り返すことで, シナリオテストが成立します。  
Scenamatica はこのようなテストの需要に最適化されているのです。

## 例を用いた説明 {#example}

例えば, とある小さなプラグインについて考えます。  
...
>   発火条件で指定される, シナリオの遂行に必要な村人をスポーンさせるために使います。
>   ```yaml
>   context:
>     entities:
>     - type: villager
