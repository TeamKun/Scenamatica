...
シナリオは, プラグインのテスト方法をフローで定義したものです。 
プラグインのテスト対象機能の発火・検証方法, およびそのテストを実行するための前提環境を, １つのファイル（シナリオファイルという）に記述します。  
Scenamatica は, プラグインに同梱されたシナリオファイルを読み込み, その内容に沿ってプラグインの機能を発火させ, 状態の遷移およびそれによる結果を検証・報告します。

テストを実行するための前提環境とは, テストに登場する[エンティティ](/docs/use/scenario/elements#entity)
やプレイヤ（=[アクタ](/docs/use/scenario/elements#actor)）, テストのためのワールド（=[ステージ](/docs/use/scenario/elements#stage)）の３つを指します。
これらは, 同ファイルに定義する[コンテキスト](/docs/use/scenario/elements#context)機能によって提供されます。  

## シチュエーション例を用いた説明 {#example}

例えば, とある小さなプラグインについて考えます。  
このプラグインは, 「プレイヤが村人をキルしたときに」（**発火条件**という） 
-> 「ダイアモンドがドロップする機能」（**目標動作**という）を実現しており, 今回はこの機能をテストしたいとします。
...
Scenamatica はこのようなテストの需要に最適化されています。
