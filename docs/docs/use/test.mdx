...
## シナリオ {#integrating-scenamatica-to-development-flow}

テストを始めるには, まずテストをしたいプラグインの**機能ごとに** シナリオファイルを記述する必要があります。

Scenamatica はあなたが作成した機能の発火条件や, それらにどのような結果が期待されているかを知りせん。
ですから, プラグインの開発者が, プラグインの正しい動作（期待される動作）を Scenamatica に教える必要があります。  
...
![テスト結果#600x](./images/test-result.png)

これは, テストの実行結果の種別と, 詳細ないくつかの情報を含めたものです。
種別は以下の列挙型の値を取ります。
...
>   これについての詳細は[こちら](/docs/use/scenario/elements#scenario-type-action-expect)を参照してください。
> + `SCENARIO_TIMED_OUT`  
...
>   [変数・定数の定義と参照](/docs/use/scenario/variables-and-definitions)機能によって参照された変数や定数が解決できなかったことを示します。
> 
> + `RUN_TIMED_OUT`  
...
:::tip SCENARIO_TIMED_OUT と RUN_TIMED_OUT の違いについて
