...
シナリオファイルの先頭には、対応している Scenamatica のバージョン\([`scenamatica`](/references/types/#apex-scenamatica)) を記述します。  
...
名前\([`name`](/references/types/#apex-name)) は、シナリオの識別子として使用されます。プラグインの中で一意な命名をしてください。  
説明\([`description`](/references/types/#scenario-file)) は、人間が読みやすいような説明文を記述します。
...
シナリオが実行されるためには、発火のための[トリガ](/docs/use/scenario/trigger)が必要です。  
トリガは、 [`on`](/references/types/#apex-on) というキーでオブジェクトのリストで記述します。
