...
# このシナリオでは、プレイヤが敵を倒したときに、プレイヤの真下のブロックをダイヤモンドブロックに置換されるかどうかをテストします。
scenamatica: "0.0.1"
name: "placing-diamond-block-on-kill-test"
...
- type: expect
  action: block_place
  with:
    # プレイヤの位置のブロックがダイヤモンドブロックに置換されることを期待する。
    location:
      # プレイヤの座標を指定する。
      x: 0
      y: 3
...
各シナリオの出力は、その[アクションの出力](/docs/use/scenario/actions#action-outputs)と同じです。
