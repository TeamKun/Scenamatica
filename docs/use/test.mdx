...
[シナリオ](scenario)に[実行条件](scenario/elements#conditional-execution)を指定することで、テストの実行に条件を持たせられます。
...
+ `ACTION_EXPECTATION_JUMPED`
  ...
  これについての詳細は[こちら](scenario/elements#scenario-type-condition-require:~:text=後に実行されるべきアクションが繰り上げられて先に実行された場合、このシナリオは失敗します。)
  を参照してください。
+ `SCENARIO_TIMED_OUT`  
