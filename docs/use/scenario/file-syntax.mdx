...
シナリオの実行に必要な環境を定義します。  
コンテキストの概念についての説明は[こちら](elements/#context)を、ファイルに記述する際の構文にについては[こちら](#context-structure)を参照してください。
...
シナリオを実行する条件をアクションで定義します。詳しくは[こちら](elements#conditional-execution)を参照してください。
...
[コンテキスト](elements/#context)は、以下の構造を持ちます。
...
アクターの概念についての説明は[こちら](elements/#actor)を参照してください。

### `entities` {#context-entities}

シナリオの実行に必要なエンティティを定義します。  
これを設定すると、シナリオの実行前に[ステージ](elements/#stage)内にエンティティが生成され、終了時に削除されます。

エンティティの概念についての説明は[こちら](elements/#entity)を参照してください。

### `stage` {#context-stage}

シナリオの実行に必要なステージを定義します。  

ステージの概念についての説明は[こちら](elements/#stage)を参照してください。
...
[`PlayerJoinEvent`](actions/player#join) をテストする場合、この値を `false` に設定することで
余計な発火を防止できます。
