...
真偽値を指定した場合、 [こちら](/docs/use/getting-started/configuration#actor-defaultOP) で設定したデフォルトの OP 権限レベルが参照されます。
...
[こちら](/docs/use/getting-started/configuration#actor-defaultPermissions) で設定したデフォルトの権限が予め適用されています。  
上記の設定を取り消すことはできません。
