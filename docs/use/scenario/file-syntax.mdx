...
アクターの概念についての説明は[こちら](elements/#actor-structure)を参照してください。

### `entities` {#context-entities}

シナリオの実行に必要なエンティティを定義します。  
これを設定すると、シナリオの実行前に[ステージ](elements/#stage-structure)内にエンティティが生成され、終了時に削除されます。

### `stage` {#context-stage}

シナリオの実行に必要なステージを定義します。  

ステージの概念についての説明は[こちら](elements/#stage-structure)を参照してください。
