...
この変数を使用してシナリオを取得するには、子に[シナリオの名前](use/scenario/types/#apex-name)を指定します(MUST)。
...
<Object objects={[
  ...
  {
    name: "scenarios.*.output",
    type: "Object",
    description: "シナリオの出力です。",
    anchor: "variables-type-session-scenarios-output"
  },
  {
    name: "scenarios.*.runif.output",
    type: "Object",
    description: "シナリオの条件の出力です。",
    anchor: "variables-type-session-scenarios-runif-output"
  },
  {
    name: "trigger",
    type: "Trigger",
...
#### `session`.`scenarios`.`*`.`output` {#variables-type-session-scenarios-result}

`*` には、シナリオのインデックスまたは[名前](use/scenario/types/#scenario-name)を指定します。

#### `session`.`scenarios`.`*`.`runif` {#variables-type-session-scenarios-runif-output}
...
<Object objects={[
  ...
  },
  {
    ...
    type_link: "/docs/use/scenario/test#test-status-result",
