...
`true` を指定した場合、投射物の着弾イベントのみを発生させます。

通常は、以下の動作を行います。

+ `hitEntity` が指定されている場合、そのエンティティの位置に投射物をテレポートさせ、速度を `0` にします。
+ `hitBlock` が指定されている場合、そのブロックの `blockFace` の正面 `0.2` の位置に投射物をテレポートさせ、ブロックの面に対して垂直な速度を `10` にします。
