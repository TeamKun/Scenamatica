import Action, {ScenarioType} from "/src/components/Action";
import BelowDocument from "/src/components/BelowDocument";
import {ObjectType} from "/src/components/Object";


# アクション

シナリオの有効なアクションの一覧です。

---

:::tip

**シナリオやシナリオファイルの概念が初めての方は[こちら](..)を参照して先に理解しておいてください。**

:::

## 概要 {#overview}

アクションとは、 Minecraft で実行できる動作を一つ一つに細かく分解したものです。
これを複数組み合わせ、フローのように組み立てたものが[シナリオ](../elements#scenario)です。


すべてのアクションは、次の３つの[シナリオタイプ](../elements#scenario-type)の一つ以上で使用できます。  
+ [アクション実行(`execute`)](../elements#scenario-type-action-execution) - アクションを実行します。  
  例：プレイヤをキルする、プレイヤにメッセージを送信する
+ [アクション実行期待(`expect`)](../elements#scenario-type-expect) - アクションが**実行されるか**を検知または実行されるまで待機します。  
  例：プレイヤが死亡する、プレイヤにメッセージが送信される
+ [条件要求(`require`)](../elements#scenario-type-condition-require) - アクション（状態）が**すでに**満たされているかを検知します。  
  例：プレイヤが死亡している、プレイヤがメッセージを送信している

そのため、アクション一つにつき最大で３つのふるまいを持ちます。  
例えばアクション [PlayerDeathAction](player/#death) は以下の３つのふるまいを持ちます。

| シナリオタイプ                                                          | ふるまい        |
|:-----------------------------------------------------------------|:------------|
| [アクション実行(`execute`)](../elements#scenario-type-action-execution) | プレイヤをキルする   |
| [アクション実行期待(`expect`)](../elements#scenario-type-expect)          | プレイヤが死亡する   |
| [条件要求(`require`)](../elements#scenario-type-condition-require)   | プレイヤが死亡している |


## アクションの種類 {#actions}

アクションは膨大な数の種類があるので、各カテゴリ別に分けられています。  
以下のアンカをクリックすると、そのカテゴリのアクション一覧に遷移します。

+ [ブロック関連](block)  
  ブロックの破壊など
+ [プレイヤ関連](player)  
  プレイヤの死亡、ゲームモードの変更など
+ [サーバ関連](server)  
  コマンドの送信など
+ [ワールド関連](world)  
  ワールドボーダの変更など
+ [エンティティ関連](entity)  
  エンティティのダメージなど

## 特にページを分ける程でもないアクション {#general-actions}

### 特別：マイルストーンの達成 {#milestone}

:::tip

マイルストーンシステムの概要と詳しい使い方は以下のページを参照してください。

<BelowDocument docId="use/scenario/milestone"
               label="マイルストーンシステムの概要"
/>

:::

<Action name="マイルストーンの達成"
        description="Scenamatica マイルストーンを達成したことを検知/設定します。"
        id="milestone"
        args={[
          {
            name: "name",
            description: "マイルストーンの名前です。",
            type: ObjectType.STRING,
            required: [ScenarioType.EXECUTE, ScenarioType.REQUIRE]
          },
          {
            name: "reached",
            description: "マイルストーンを達成したかどうかを設定します。",
            type: ObjectType.BOOLEAN,
            required: [ScenarioType.EXECUTE, ScenarioType.REQUIRE]
          }
        ]}
        executable requireable watchable
/>

### 特別：チャットメッセージの送信 {#message}

<Action name="メッセージの送信"
        description="エンティティにメッセージを送信します。"
        id="message"
        args={[
          {
            name: "message",
            anchor: "message-message",
            type: ObjectType.STRING,
            description: "メッセージの内容",
            required: [ScenarioType.EXECUTE]
          },
          {
            name: "recipient",
            type: "プレイヤ",
            description: "メッセージの受信者",
            required: [ScenarioType.EXECUTE]
          }
        ]}
        executable requireable watchable
/>

#### `message` {#message-message}

メッセージの色及び書式は 接頭辞 `§` に続けて[特定の文字](https://minecraft.fandom.com/ja/wiki/Formatting_codes)を指定します。

### 特別：反転

<Action name="反転"
        description="条件の評価結果を反転します。"
        id="negate"
        args={[
          {
            name: "action",
            type: "アクション",
            description: "反転するアクション",
            required: [ScenarioType.REQUIRE]
          },
          {
            name: "with",
            type: "オブジェクト",
            description: "反転するアクションの引数",
            required: [ScenarioType.REQUIRE]
          }
        ]}
        requireable
/>

#### 例

```yaml

# プレイヤが死亡していない場合に, そのプレイヤをキルします。
- type: execute
  action: player_death
  with:
    target: Actor001
  runif:
    action: negate
    with:
      action: player_death
      with:
        target: Actor001

```
