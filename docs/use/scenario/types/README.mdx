...
定義機能に関しては、[定義と参照 機能の概要](/docs/use/scenario/variables-and-definitions/variables-and-definitions#definition) を参照してください。
...
条件に一致しなかった場合、テストは `SKIPPED` で終了し、失敗とはみなされません。
...
:::caution
これを指定する場合、[Entity 型](/docs/use/scenario/types/entities#entity-structure) の [`type`](/docs/use/scenario/types/entities#entity-type) を
必ず指定する必要があります。  
...
`false` の場合、アクターは準備されますがログイン処理は行われません。

[`PlayerJoinEvent`](/docs/use/scenario/actions/player#join) をテストする場合、この値を `false` に設定することで
余計な発火を防止できます。
