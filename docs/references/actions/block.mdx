...
対象となったブロックの [`Block`](/references/types/misc#block) オブジェクトを出力します。

## 全般 - 出力 `player`

その操作を行ったプレイヤの [`Player`](/references/types/entities#player) オブジェクトを出力します。
...
:::warning
実行シナリオでは、 `block`の[`type`](/references/types/misc#block) を空にできません。  
