...
+ [エンティティに関連する型](/references/types/entities)
+ [インベントリとアイテムに関連する型](/references/types/inventory-items)
+ [その他の型](/references/types/misc)  
  [`Location`](/references/types/misc#location) や
  [`Block`](/references/types/misc#block),
  [`NamespacedKey`](/references/types/misc#namespaced-key) など
...
<Object objects={[
  ...
  {
    name: "minecraft",
    anchor: "apex-minecraft",
    type: "MinecraftVersion",
    type_anchor: "minecraft-version",
    description: "Scenamatica のバージョンです。",
    required: true
  },
  {
...
#### `minecraft` {#apex-minecraft}

シナリオテスト時に使用できる 使用できる Minecraft バージョンの範囲を指定します。

#### `name` {#apex-name}

シナリオのプラグイン内での一意の名前です、主に、 Scenamatica がシナリオを識別するために使用したり、**手動実行のコマンドの引数
**に使用されます。
...
[実行順序](/docs/use/scenario/#scenari-ordering)は、数値が低いものから順に実行されます。未設定の場合は自動的に名前順でソートされ、セッションの末尾に追加されます。

#### `context` {#apex-context}

シナリオの実行に必要な環境を定義します。  
コンテキストの概念についての説明は[こちら](/docs/use/scenario/elements#context)
を、ファイルに記述する際の構文にについては[こちら](#context)を参照してください。
...
シナリオを実行する条件をアクションで定義します。詳しくは[こちら](/docs/use/scenario/elements#conditional-execution)
を参照してください。
...
## Minecraft バージョン 型 {#minecraft-version}

<Object
  objects={[
    {
      name: "since",
      description: "サポートされる Minecraft バージョンの範囲の開始です。",
      type: PrimitiveTypes.STRING,
      default: "any"
    },
    {
      name: "until",
      description: "サポートされる Minecraft バージョンの範囲の終了です。",
      type: PrimitiveTypes.STRING,
      default: "any"
    }
  ]}
...
:::warning
これを指定する場合は、[Entity 型](/references/types/entities#entity)
の [`type`](/references/types/entities#entity-type) を
必ず指定する必要があります。  
...
それぞれの[トリガ](/docs/use/scenario/trigger)の説明を参照してください。
...
有効なトリガの種類の一覧は[こちら](/docs/use/scenario/trigger#triggers)を参照してください
...
[アクションの種類](/references/actions/#actions)を指定します。
